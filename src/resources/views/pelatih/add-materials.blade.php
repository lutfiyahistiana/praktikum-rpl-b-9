    ...
    {{-- ========================== MAIN CONTENT ========================== --}}
        <main class="flex-1 px-4 sm:px-6 lg:px-8 pb-6 space-y-6">
            ...
            <form id="form-tambah-materi" action="{{ route('pelatih.materials.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                {{-- Card: Upload File --}}
                <div class="bg-white border border-[#E6E6E6] rounded-lg p-6">
                    <h2 class="text-lg font-bold text-gray-900 mb-4">Upload File Materi</h2>
                    ...
                    {{-- File Preview List (muncul setelah upload) --}}
                    <div id="file-preview-list" class="mt-4 space-y-2 hidden"></div>
                </div>

                {{-- Action Buttons --}}
                <div class="flex flex-col sm:flex-row gap-3">
                    <button type="submit" id="btn-simpan-materi" class="w-full sm:w-auto sm:px-8 bg-[#008CFF] hover:bg-[#006FCC] text-white font-medium text-sm py-2.5 rounded-lg transition-colors shadow-sm text-center">
                        Simpan Materi
                    </button>
